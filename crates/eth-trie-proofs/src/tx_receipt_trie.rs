...
impl TxReceiptsMptHandler {
    ...
    pub async fn tx_hash_to_tx_index(&self, tx_hash: B256) -> Result<u64, EthTrieError> {
    ...
    pub async fn build_tx_receipt_tree_from_tx_hash(&mut self, tx_hash: B256) -> Result<(), EthTrieError> {
    ...
    pub async fn build_tx_receipts_tree_from_block(&mut self, block_number: u64) -> Result<(), EthTrieError> {
        let (txs, tx_receipt_root) = self.provider.get_block_transaction_receipts(block_number).await?;
    ...
    pub fn build_trie(&mut self, tx_receipts: Vec<ConsensusTxReceipt>, expected_root: B256) -> Result<(), EthTrieError> {
        ...
        if trie.root_hash()? != expected_root {
    ...
    pub fn verify_proof(&self, tx_index: u64, proof: Vec<Vec<u8>>) -> Result<Vec<u8>, EthTrieError> {
        let target_trie = self.trie.as_ref().ok_or(EthTrieError::TrieNotFound)?;
        match target_trie
            .trie
            .verify_proof(target_trie.root, alloy_rlp::encode(U256::from(tx_index)).as_slice(), proof)
        {
    ...
    pub fn get_tx_receipt(&self, tx_index: u64) -> Result<ConsensusTxReceipt, EthTrieError> {
        let target_trie = self.trie.as_ref().ok_or(EthTrieError::TrieNotFound)?;
        target_trie.elements.get(tx_index as usize).ok_or(EthTrieError::TxNotFound).cloned()
    }

    /// Retrieves all elements within the trie.
    pub fn get_elements(&self) -> Result<Vec<ConsensusTxReceipt>, EthTrieError> {
        let target_trie = self.trie.as_ref().ok_or(EthTrieError::TrieNotFound)?;
...
mod tests {
    ...
    async fn test_tx_receipt_byzantium() {
        ...
        tx_receipts_mpt_handler.build_tx_receipts_tree_from_block(4370000).await.unwrap();

        let tx_index = tx_receipts_mpt_handler.tx_hash_to_tx_index(target_tx_hash).await.unwrap();
        let proof = tx_receipts_mpt_handler.get_proof(tx_index).unwrap();
        tx_receipts_mpt_handler.verify_proof(tx_index, proof.clone()).unwrap();
    }
    ...
    async fn test_tx_receipt_2930() {
        ...
        let mut tx_receipts_mpt_handler = TxReceiptsMptHandler::new(url).unwrap();
        tx_receipts_mpt_handler.build_tx_receipts_tree_from_block(12244000).await.unwrap();
    ...
    async fn test_tx_receipt_1559() {
        ...
        let mut tx_receipts_mpt_handler = TxReceiptsMptHandler::new(url).unwrap();
        tx_receipts_mpt_handler.build_tx_receipts_tree_from_block(12965000).await.unwrap();
    ...
    async fn test_tx_receipt_4844() {
        ...
        let mut tx_receipts_mpt_handler = TxReceiptsMptHandler::new(url).unwrap();
        tx_receipts_mpt_handler.build_tx_receipts_tree_from_block(19426589).await.unwrap();

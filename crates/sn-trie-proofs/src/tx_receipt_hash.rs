...
pub fn calculate_receipt_hash(receipt: &TransactionAndReceipt<Felt>, l1_gas: u64) -> Felt {
    ...
    let common_properties = match &receipt.receipt {
        starknet_types_rpc::TxnReceipt::Declare(tx_receipt) => &tx_receipt.common_receipt_properties,
        starknet_types_rpc::TxnReceipt::Deploy(tx_receipt) => &tx_receipt.common_receipt_properties,
        starknet_types_rpc::TxnReceipt::DeployAccount(tx_receipt) => &tx_receipt.common_receipt_properties,
        starknet_types_rpc::TxnReceipt::Invoke(tx_receipt) => &tx_receipt.common_receipt_properties,
        starknet_types_rpc::TxnReceipt::L1Handler(tx_receipt) => &tx_receipt.common_receipt_properties,
    };

    hash_chains.push(common_properties.transaction_hash);
    hash_chains.push(common_properties.actual_fee.amount);
    hash_chains.push(calculate_messages_sent_hash(&common_properties.messages_sent));
    ...
    hash_chains.push(common_properties.execution_resources.data_availability.l1_data_gas.into());
